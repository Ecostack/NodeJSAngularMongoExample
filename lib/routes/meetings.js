...
router.get('/upcoming', function (req, res, next) {
    meetingService.getUpcomingMeetings().then(function(pResults) {
        res.json(pResults).end();
    }).catch(util.asyncCatch(res));
});

router.get('/averageAmountUpcoming', function (req, res, next) {
    meetingService.getAverageAmountOfUpcomingMeetings().then(function(pResults) {
        res.json(pResults).end();
    }).catch(util.asyncCatch(res));
});

router.get('/nextDateForPerson', function (req, res, next) {
    meetingService.getDateOfEachPerson().then(function(pResults) {
        res.json(pResults).end();
    }).catch(util.asyncCatch(res));
});
